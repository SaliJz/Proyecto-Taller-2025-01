...
public class EnemigoDisparador : MonoBehaviour
{
    ...
    // Velocidad con la que se acerca al jugador
    ...
    private void Update()
    {
        ...
        // Calcular la dirección y distancia horizontal hacia el jugador (ignorando el eje Y)
        ...
        // Si el enemigo está fuera de la distancia deseada, se acerca suavemente
        if (distanciaAlJugador > distanciaDeseada)
        {
            Vector3 nuevaPosicion = transform.position + direccionHaciaJugador.normalized * velocidadMovimiento * Time.deltaTime;
