...
public class Fase2Vida : MonoBehaviour
{
    ...
    private void TransitionToPhase(int nextPhase, Transform nextColumn)
    {
        ...
        // Detener todo lo que pudiera seguir de la fase anterior
        if (phaseCoroutine != null) StopCoroutine(phaseCoroutine);
        anticipation.StopAllCoroutines();
    ...
    private IEnumerator ExecutePhase2Sequence()
    {
        ...
        while (true)
        {
            // En ciclos posteriores, esperar intervalo antes de anticipar
            if (!firstLoop)
...
//        // 2) Espera inicial antes de la primera anticipación
...
//        while (true)
//        {
//            // En loops posteriores, esperar el intervalo de ataque antes de anticipar
//            if (!firstLoop)
//                yield return new WaitForSeconds(attackInterval);
//            firstLoop = false;
