...
//    [Header("Configuracion de la serpiente")]
//    public int cantidadSegmentosCuerpo = 10;             // Numero de segmentos de cuerpo
//    public float distanciaCabezaCuerpo = 0.5f;           // Distancia entre cabeza y primer segmento
//    public float separacionSegmentos = 0.5f;             // Distancia entre segmentos posteriores
//    public float separacionCola = 0.5f;                  // Distancia entre ultimo segmento de cuerpo y cola
//    public float velocidad = 5f;                         // Velocidad de movimiento de la cabeza
//    public float amplitudSerpenteo = 0.5f;               // Amplitud del movimiento serpenteante
//    public float frecuenciaSerpenteo = 2f;               // Frecuencia del movimiento serpenteante
//    public float ejeY = 1f;                              // Altura a la cual la serpiente se mantiene (y = 0 es suelo)
//    public float umbralDetencion = 0.1f;                 // Distancia minima para detenerse en el jugador
...
//    {
//        Vector3 posInicial = new Vector3(transform.position.x, ejeY, transform.position.z);
//        int totalSegmentos = cantidadSegmentosCuerpo + 2;
//        maxRegistroPosiciones = Mathf.CeilToInt((distanciaCabezaCuerpo + separacionSegmentos * (cantidadSegmentosCuerpo - 1) + separacionCola) * 2);
...
//        // Rellenar historial con la posicion inicial
//        for (int i = 0; i < maxRegistroPosiciones; i++)
//            posicionesCabeza.Add(posInicial);
//    }
...
//        Transform cabeza = segmentos[0];
//        Vector3 targetPos = new Vector3(jugador.position.x, ejeY, jugador.position.z);
//        float distAlJugador = Vector3.Distance(cabeza.position, targetPos);

//        if (distAlJugador > umbralDetencion)
//        {
//            MoverCabeza(targetPos);
//        }
//        else
//        {
//            // Fija cabeza
//            cabeza.position = targetPos;
//            posicionesCabeza.Insert(0, targetPos);
//            if (posicionesCabeza.Count > maxRegistroPosiciones)
//                posicionesCabeza.RemoveAt(posicionesCabeza.Count - 1);
//        }
...
//        posicionesCabeza.Insert(0, nuevaPos);
//        if (posicionesCabeza.Count > maxRegistroPosiciones)
//            posicionesCabeza.RemoveAt(posicionesCabeza.Count - 1);
//    }
...
//            if (i == segmentos.Count - 1)
//            {
//                // cola
//                distanciaAtras = distanciaCabezaCuerpo + separacionSegmentos * (cantidadSegmentosCuerpo - 1) + separacionCola;
//            }
//            else
//            {
//                // cuerpo
//                distanciaAtras = distanciaCabezaCuerpo + separacionSegmentos * (i - 1);
//            }
//            Vector3 posObjetivo = ObtenerPosicionEnTrayectoria(distanciaAtras);
//            Transform seg = segmentos[i];
//            seg.position = Vector3.Lerp(seg.position, posObjetivo, Time.deltaTime * velocidad);
