...
public class Fase1Vida : MonoBehaviour
{
    ...
    // Nuevo: tiempo máximo de espera para fase previa
    [Header("Timeout fase previa")]
    [Tooltip("Segundos a esperar sin daño antes de forzar fase previa completada.")]
    public float previaTimeout = 25f;
    ...
    void Start()
    {
        ...
        // Iniciar timeout para fase previa
        StartCoroutine(PreviaTimeoutRoutine());
    }

    // Coroutine que fuerza el fin de la fase previa tras previaTimeout segundos
    private IEnumerator PreviaTimeoutRoutine()
    {
        yield return new WaitForSeconds(previaTimeout);
        if (!fasePreviaCompleted)
        {
            Debug.Log($"[Fase1Vida] Timeout de {previaTimeout}s: forzando fase previa completada.");
            vidaFasePrevia = 0;
            CompletePhasePrevia();
        }
    }

    // Método factor común para completar fase previa
    private void CompletePhasePrevia()
    {
