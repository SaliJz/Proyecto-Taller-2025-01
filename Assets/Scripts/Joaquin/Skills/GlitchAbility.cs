...
public class GlitchAbility : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (playerCamera == null)
        {
            ...
            if (playerCamera == null) Debug.LogError("No se encontr� la c�mara principal. Aseg�rate de que haya una c�mara con la etiqueta 'MainCamera' en la escena.");
        }

        if (projectileSpawnPoint == null) Debug.LogError("Projectile Spawn Point no est� asignado en GlitchTime.");

        if (projectilePrefab == null) Debug.LogError("Projectile Prefab no est� asignado en GlitchTime.");

        if (abilityInfo == null)
        {
            abilityInfo = GetComponent<AbilityInfo>();
            if (abilityInfo == null) Debug.LogError("AbilityInfo no est� asignado en GlitchTime.");
        }
    }
