...
public class IgnitionCodeAbility : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (projectileSpawnPoint == null) Debug.LogError("Projectile Spawn Point no est� asignado en IgnitionCodeAbility.");

        if (projectilePrefab == null) Debug.LogError("Projectile Prefab no est� asignado en IgnitionCodeAbility.");

        if (abilityInfo == null)
        {
            abilityInfo = GetComponent<AbilityInfo>();
            if (abilityInfo == null) Debug.LogError("AbilityInfo no est� asignado en IgniteCodeAbility.");
        }
    }
